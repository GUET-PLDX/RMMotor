...
class RMMotor : public LibXR::Application, public Motor {
 public:
  ...
  RMMotor(LibXR::HardwareContainer& hw, LibXR::ApplicationManager& app,
    ...
    if (config_param_.id_control == M3508_M2006_CTRL_ID_BASE) {
      motor_index = 0;
      motor_num = config_param_.id_feedback - M3508_M2006_FB_ID_BASE;
    } else if (config_param_.id_control == M3508_M2006_CTRL_ID_EXTAND) {
      // M3508/M2006 扩展组和 GM6020 基础组共享 0x1ff 控制帧。
      motor_index = 1;
      motor_num = config_param_.id_feedback - M3508_M2006_FB_ID_EXTAND;
    } else if (config_param_.id_control == GM6020_CTRL_ID_EXTAND) {
      motor_index = 2;
 ...
 private:
   ...
   *
   * @details
   * 为了避免 CAN 接收中断长期占用 CPU，入队失败时最多丢弃一帧旧数据并重试一次。
   */
